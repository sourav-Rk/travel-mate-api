...
export class GenerateSignedUrlUsecase implements IGenerateSignedUrlUsecase{
    ...
    async execute(publicId: string, expiresIn: string): Promise<string> {
        ...
        if(!publicId || typeof publicId !== "string" ){
            throw new CustomError(HTTP_STATUS.BAD_REQUEST,"Missing or invalid public id");
        }
    ...
    async executeMultiple(publicIds: string[], expiresIn: number): Promise<string[]> {
        if(!Array.isArray(publicIds) || publicIds.length === 0){
            throw new CustomError(HTTP_STATUS.BAD_REQUEST,"Missing or invalid public ids array");
        }
        ...
        const urls = await Promise.all(publicIds.map(publicId => generateSignedUrl(publicId,expiresIn)));
        return urls;
    }
}